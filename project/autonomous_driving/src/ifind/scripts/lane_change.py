...
class lc_path_pub :
    def __init__(self):
        ...
        for line in lines:
            ...
            pose = PoseStamped()
            pose.pose.position.x = float(split_line[0])
            pose.pose.position.y = float(split_line[1])
    ...
    def odom_callback(self,msg):
        ...
        self.ego_pose_x = msg.pose.pose.position.x
        self.ego_pose_y = msg.pose.pose.position.y
            ...
            for i in range(len(global_vaild_object)):
                for path in ref_path.poses:
                    if global_vaild_object[i][0] == 1 or global_vaild_object[i][0] == 2:
                        ...
                        if dis < 2.5:
                            rel_distance = sqrt(pow(self.ego_pose_x - global_vaild_object[i][1], 2) + pow(self.ego_pose_y - global_vaild_object[i][2], 2))
