...
class latticePlanner:
    ...
    def checkObject(self, ref_path, object_data, vehicle_status):
        collision_distance_threshold = 2  # 기존 임계값
        min_distance_to_path = float('inf')  # 장애물과 경로 사이의 최소 거리

        for obstacle in object_data.obstacle_list:
            obstacle_point = np.array([obstacle.position.x, obstacle.position.y])

            for path_pose in ref_path.poses:
                path_point = np.array([path_pose.pose.position.x, path_pose.pose.position.y])
                distance = np.linalg.norm(path_point - obstacle_point)

                if distance < collision_distance_threshold:
                    # 장애물이 충돌 판정 거리 안에 있을 경우
                    if distance < min_distance_to_path:
                        min_distance_to_path = distance

        # 장애물이 local path와 가까운 경우 True 반환, 그렇지 않은 경우 False 반환
        return min_distance_to_path < collision_distance_threshold
    ...
    def latticePlanner(self,ref_path, vehicle_status):
        ...
        look_distance = int(vehicle_velocity * 0.1 * 2)
