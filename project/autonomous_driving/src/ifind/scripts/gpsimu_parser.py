...
class GPSIMUParser:
    def __init__(self):
        ...
        self.is_imu=False
        self.is_gps=False
        self.last_firebase_write_time = rospy.get_time()
    ...
    def navsat_callback(self, gps_msg):
        ...
        print("lat", gps_msg.latitude)
        print("lon", gps_msg.longitude)
    ...
    def firebase_write(self, lat, lon):

        # 이미 초기화된 앱이 있는지 확인합니다.
        if not firebase_admin._apps:
            # 서비스 계정 키 파일을 사용하여 Firebase Admin SDK를 초기화합니다.
    ...
    def convertLL2UTM(self):       
        ...
        # # firebase에 gps 쓰기 주기 설정
        # if current_time - self.last_firebase_write_time >= 0.3:
        #     self.firebase_write(self.lat, self.lon)
        #     self.last_firebase_write_time = current_time  # 마지막 호출 시간 업데이트
        #     rospy.loginfo('Latitude: %s, Longitude: %s', self.lat, self.lon)
