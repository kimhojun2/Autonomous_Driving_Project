...
class dijkstra_path_pub :
    ...
    def goal_callback(self):
        ...
        if doc.exists:
            ...
            # select 필드에 값이 있는 항목 찾기
            for node in data_list:
                if node['select']:  # select 필드에 값이 있으면
                    lat = node['latitude']
                    lon = node['longitude']

                    # 위도와 경도를 UTM 좌표계로 변환
                    x, y = self.convert_latlon_to_xy(lon, lat)
                    print(f"Converted X: {x}, Y: {y}")

                    self.selected_nodes.append({'number' : node['number'], 'x' : x, 'y' : y})

            

                    # nearest_node_id = self.find_nearest_node_idx(x, y)

                    # self.end_node = nearest_node_id
    ...
    def calc_dijkstra_path_node(self, start_node, end_node):
        ...
        # Assuming path['point_path'] contains the list of points in the path
