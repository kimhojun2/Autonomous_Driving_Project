...
# 해당 예제에서는 차선 변경 시작 점과 끝점 사이를 직선 으로 연결하여 차량이 주행 할 경로를 만듭니다.
...
class lc_path_pub :
    def __init__(self):
        ...
        lc_1_path = pkg_path + '/path' + '/lc_1.txt'
        with open(lc_1_path, 'r') as f:
            lines = f.readlines()
            for line in lines:
                values = line.split()  # 문자열을 공백으로 분리하여 리스트로 만듦
                x, y = values[:2]  # 리스트의 첫 두 요소를 x, y에 할당
                # x, y = line.split()
                pose = PoseStamped()
        ...
        lc_2_path = pkg_path + '/path' + '/lc_2.txt'
        with open(lc_2_path, 'r') as f:
            lines = f.readlines()
            for line in lines:
                values = line.split()  # 문자열을 공백으로 분리하여 리스트로 만듦
                x, y = values[:2]  # 리스트의 첫 두 요소를 x, y에 할당
    ...
    def getLaneChangePath(self, ego_path, lc_path, start_point, end_point, start_next_point, end_waypoint_idx):
        ...
        # 차선변경 시작점과 끝점 사이의 거리 계산
        dx = end_point.pose.position.x - start_point.pose.position.x
        dy = end_point.pose.position.y - start_point.pose.position.y
        start_path_distance = sqrt(pow(dx,2) + pow(dy,2))
