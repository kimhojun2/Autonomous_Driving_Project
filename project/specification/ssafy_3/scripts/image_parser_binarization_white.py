...
class IMGParser:
    ...
    def callback(self, msg):
        ...
        '''
        # 하얀색으로 설정해두면 차선이 인식이 안됌(차선이 베이지색)
        lower_wlane = np.array([0, 30, 180])
        upper_wlane = np.array([30, 80, 255])
        # 흰색
        ...
        img_concat2 = np.concatenate((img_bgr, img_hsv), axis=1)
        ...
        '''
        cv2.imshow("Image window", img_concat)
        # cv2.imshow("Image window", img_concat2)
        cv2.waitKey(1)
