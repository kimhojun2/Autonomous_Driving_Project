...
class pure_pursuit :
    ...
    def calc_pure_pursuit(self,):
        ...
        trans_matrix = np.array([[cos(self.vehicle_yaw), -sin(self.vehicle_yaw), self.current_postion.x],
        ...
        for num,i in enumerate(self.path.poses) :
            ...
            if local_path_point[0]>0 :
                dis = sqrt(local_path_point[0]**2 + local_path_point[1]**2)  # Calculate distance to the point
                if dis >= self.lfd :
...
class velocityPlanning:
    ...
    def curvedBaseVelocity(self, gloabl_path, point_num):
        ...
        for i in range(point_num, len(gloabl_path.poses) - point_num):
            ...
            # # Pseudo Inverse를 사용하여 곡률 계산
            # A = np.array(x_list)
            # b = np.array(y_list)

            # # Pseudo Inverse 계산
            # A_inv = np.linalg.pinv(A)
            # coeffs = np.dot(A_inv, b)

            # # 곡률 반경 계산
            # r = np.sqrt(coeffs[0]**2 + coeffs[1]**2)
