...
class pure_pursuit :
    ...
    def calc_pure_pursuit(self,):
        ...
        # 
        current_speed = self.status_msg.velocity.x  # Assuming this is in m/s
...
class velocityPlanning:
    ...
    def curvedBaseVelocity(self, gloabl_path, point_num):
        ...
        for i in range(point_num, len(gloabl_path.poses) - point_num):
            ...
            # A = np.vstack([x_list, y_list, np.ones(len(x_list))]).T
            # B = np.array(y_list)**2 + np.array(x_list)**2
            # coeff, _, _, _ = np.linalg.lstsq(A, B, rcond=None)
            # xc = -0.5 * coeff[0]
            # yc = -0.5 * coeff[1]
            # r = sqrt(coeff[2] + xc**2 + yc**2)
            ...
            v_max = sqrt(self.road_friction * 9.81 * r)
