...
# 내가 짠 코드
        ...
        self.global_path_pub = rospy.Publisher('/lane_change_path', Path, queue_size=1)
...
# 내가 짠 코드
        # Load lc_1.txt
        lc_1_path = os.path.join(pkg_path, 'path', 'lc_1.txt')
        with open(lc_1_path, 'r') as f:
            lines = f.readlines()
            for line in lines:
                data = line.strip().split()
                x, y = data[0], data[1]  # 첫 번째와 두 번째 값만 사용
                # 나머지 데이터는 무시하거나 필요에 따라 처리
                pose = PoseStamped()
        ...
        # Load lc_2.txt
        lc_2_path = os.path.join(pkg_path, 'path', 'lc_2.txt')
            ...
            for i in range(len(global_vaild_object)):
                for path in ref_path.poses:
                    if global_vaild_object[i][0] == 1 or global_vaild_object[i][0] == 2:
                        ...
                        if dis < 2.5:
                            rel_distance = dis  # 예제에서는 단순히 거리를 사용
                            if rel_distance < min_rel_distance:
